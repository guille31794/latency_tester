...
% Reduce verbatim font size to avoid overfull hbox in code blocks
\let\origverbatim\verbatim
\renewcommand{\verbatim}{\small\origverbatim}
...
Durante la lectura de este documento, será presentado con todo detalle un dispositivo electrónico de nuevo desarrollo, cuyas aplicaciones serán especialmente útiles en el ámbito militar y de la seguridad de entornos en tiempo real, pero que también puede tener aplicaciones en otros sectores, como la industria, donde puede ayudar a cuantificar y estandarizar una medida como es el retardo de vídeo.
...
A su vez, un sistema de videovigilancia es crítico cuando tiene que funcionar “en tiempo real”, es decir, ser capaz de captar una imagen, y mostrarla en un monitor con un tiempo de latencia suficientemente bajo como para que el operador de cámaras no perciba disonancia entre las entradas que recibe la cámara y el tiempo que tarda esta en dar una respuesta a dichas entradas.
...
Se contextualiza este concepto en el fenómeno que se conoce como revolución digital, cuyo avance ha afectado enormemente a los dispositivos y medios de captación de imágenes y su posterior tratamiento y adecuación al medio en el que serán utilizadas.
...
Asimismo, se realizará una descripción de toda la metodología aplicada al realizar las mediciones, un completo análisis del transfondo tecnológico del hardware utilizado durante el proyecto y se documentará el desarrollo e instalación del software que constituye el sistema de medición.
...
Dado el actual auge de los sistemas digitales en todos los posibles ámbitos de la vida cotidiana, hay uno que ha destacado sobremanera: la captación, tratamiento y visualización de las imágenes y, por ende, del vídeo.

Aplicada esta revolución digital al entorno de la defensa y seguridad, se llega a los sistemas de videovigilancia modernos, los cuales aumentan la complejidad de los antiguos sistemas analógicos, añadiendo componentes electrónicos de diversa índole que son los encargados de conseguir una imagen digital de calidad, pero que a su vez producen inherentemente un retardo en la consecución y retransmisión de la misma a través de un monitor.
...
Esto lleva hasta la idea de este estudio, el cual pretende proporcionar una herramienta sencilla y económica para medir dichos retardos y, a la vez, establecer una serie de pautas generales para identificar qué sistemas pueden ser considerados como críticos y cuáles no, precisamente con base en este criterio de medida que es su latencia de funcionamiento.

Se comenzará el desarrollo del sistema con la premisa de desarrollar un prototipo de producto funcional, el cual disponga entre sus características técnicas:
...
Estos dispositivos, que son los responsables de las mediciones en el sistema de Voysys, serán los encargados tanto de tomar mediciones como de realizar los cálculos y representar los resultados para su visualización. Es por ello, y por la condición de portátil del dispositivo, por lo que se realizarán los siguientes cambios en los componentes:
...
Respecto a la medición de latencias en sistemas de visión nocturna, se debe tener en cuenta que existe un amplio abanico de tecnologías disponibles actualmente, partiendo de las tradicionales con iluminación fosfórica verde hasta las modernas imágenes térmicas digitales. Pese a ello, no se ha encontrado nada similar al dispositivo que pretende desarrollar este proyecto. Teniendo en cuenta que todo lo relacionado con este ámbito de la vigilancia nocturna suele pertenecer al ámbito militar, es comprensible que haya menos información al respecto.
...
Se dará por concluido el proyecto cuando se tenga desarrollado el software del medidor de latencias con un funcionamiento correcto y adecuado, validado tanto en el entorno de desarrollo como en emulación de la plataforma destino (ARM64), y se disponga de la infraestructura de compilación cruzada necesaria para generar el binario de producción desplegable sobre la Raspberry Pi.

Respecto a los dispositivos videográficos, se han mostrado de forma introductoria y no serán cubiertos en primera instancia por el alcance de este proyecto.

\section{Glosario}

\begin{description}
    \item [{Apertura}] Indica la luminosidad del objetivo que incorpora la cámara.
    \item [{Autogating}] Técnica de alimentación pulsada en dispositivos de visión nocturna que enciende y apaga el intensificador a alta frecuencia para evitar la saturación por fuentes de luz intensa.
    \item [{Balance de blancos}] Ajuste de la cámara que sirve para equilibrar el nivel de los colores básicos, rojo verde y azul.
    \item [{Baseline}] Valor de referencia obtenido durante la calibración, utilizado como umbral base para la detección de cambios en las lecturas del sensor.
    \item [{Blooming}] Efecto de saturación lumínica en sensores de imagen que provoca la pérdida parcial o total de la imagen por sobreexposición.
    \item [{Buffering}] Almacenamiento temporal de datos en memoria intermedia para compensar diferencias de velocidad entre emisor y receptor.
    \item [{Bump-in-the-wire}] Dispositivo de seguridad insertado de forma transparente en un enlace de comunicaciones sin alterar la topología de la red.
    \item [{Cut-through}] Modo de conmutación en switches de red que reenvía el paquete antes de recibirlo completamente, minimizando la latencia.
    \item [{Debug}] Modo de compilación que incluye información de depuración para facilitar la localización de errores en el código fuente.
    \item [{Deploy / Deployment}] Proceso de distribución e instalación de un software en su entorno de ejecución final.
    \item [{Flag}] Variable booleana utilizada como indicador de estado o señalización entre hilos de ejecución.
    \item [{Fotones}] Partículas con energía lumínica.
    \item [{Framework}] Conjunto de bibliotecas, herramientas y convenciones que proporcionan una estructura base para el desarrollo de aplicaciones.
    \item [{Framerate}] Tasa de fotogramas por segundo a la que un sistema de vídeo captura o reproduce imágenes.
    \item [{GOP (Group of Pictures)}] Grupo de fotogramas consecutivos en una secuencia de vídeo comprimido que comparten una estructura de predicción común.
    \item [{Hardware}] Conjunto de componentes físicos y electrónicos que conforman un sistema informático.
    \item [{ISO}] Sensibilidad que tiene el sensor de la cámara para captar luz.
    \item [{Jitter}] Variación en el retardo de llegada de paquetes de datos en una red, que puede causar irregularidades en la reproducción de vídeo.
    \item [{LED (Light Emitting Diode)}] Diodo emisor de luz utilizado como fuente de estímulo luminoso controlado.
    \item [{Open source}] Modelo de desarrollo de software cuyo código fuente es de libre acceso, modificación y distribución.
    \item [{Offscreen}] Modo de renderizado gráfico sin pantalla física conectada, utilizado en entornos de pruebas automatizadas.
    \item [{Pipeline}] Cadena secuencial de etapas de procesamiento por las que pasa una señal o dato desde su origen hasta su destino.
    \item [{Polling}] Técnica de muestreo periódico en la que el procesador consulta repetidamente el estado de un dispositivo hasta obtener una respuesta.
    \item [{Release}] Modo de compilación optimizado para producción, sin información de depuración y con optimizaciones de rendimiento activas.
    \item [{Resolución}] Cantidad de detalles que pueden apreciarse en una imagen. Medida en píxeles.
    \item [{Sensor-to-screen}] Latencia extremo a extremo medida desde la captura en el sensor de la cámara hasta la presentación en el monitor del operador.
    \item [{Signal / Slot}] Mecanismo de comunicación entre objetos en Qt que permite la emisión de eventos (signals) y su recepción por funciones suscritas (slots).
    \item [{Singleton}] Patrón de diseño que garantiza la existencia de una única instancia de una clase durante toda la ejecución del programa.
    \item [{Software}] Conjunto de programas, instrucciones y datos que permiten el funcionamiento de un sistema informático.
    \item [{Store-and-forward}] Modo de conmutación en switches de red que almacena el paquete completo antes de reenviarlo, introduciendo mayor latencia pero permitiendo verificación de errores.
    \item [{Streaming}] Transmisión continua de datos multimedia (audio o vídeo) que permite su reproducción simultánea a la recepción.
    \item [{Stub}] Implementación simulada de un componente hardware o software utilizada en pruebas unitarias para aislar el código bajo test.
    \item [{Timecode}] Marca temporal insertada en la señal de vídeo que permite identificar el instante exacto de captura de cada fotograma.
    \item [{Timeout}] Tiempo máximo de espera configurado para una operación antes de considerarla fallida.
    \item [{Toolchain}] Conjunto de herramientas de compilación (compilador, enlazador, bibliotecas) configuradas para generar código ejecutable para una arquitectura específica.
    \item [{Whitepaper}] Documento técnico publicado por un fabricante o institución que describe en detalle una tecnología, metodología o producto.
    \item [{Widget}] Componente visual de interfaz gráfica de usuario (botón, deslizador, campo de texto, etc.).
...
\begin{acronym}
  ...
  \acro{SBC}{Ordenador en una única placa}
  \acro{ARM}{Máquina RISC Avanzada}
  ...
  \acro{I2C}{Inter-Integrated Circuit - Bus de comunicación serie entre circuitos integrados}
  \acro{ADC}{Analog-to-Digital Converter - Conversor analógico-digital}
  \acro{FPGA}{Field-Programmable Gate Array - Matriz de puertas programable en campo}
  \acro{NTP}{Network Time Protocol - Protocolo de tiempo de red}
  \acro{PTP}{Precision Time Protocol - Protocolo de tiempo de precisión}
  \acro{ISP}{Internet Service Provider - Proveedor de servicios de Internet}
  \acro{LAN}{Local Area Network - Red de área local}
  \acro{UTP}{Unshielded Twisted Pair - Par trenzado sin apantallar}
  \acro{JSON}{JavaScript Object Notation - Notación de objetos JavaScript}
  \acro{API}{Application Programming Interface - Interfaz de programación de aplicaciones}
  \acro{VNC}{Virtual Network Computing - Computación virtual en red}
  \acro{SSH}{Secure Shell - Intérprete de comandos seguro}
  \acro{LED}{Light Emitting Diode - Diodo emisor de luz}
  \acro{IP}{Internet Protocol - Protocolo de Internet}
  \acro{GOP}{Group of Pictures - Grupo de imágenes}
  \acro{ASan}{AddressSanitizer - Herramienta de detección de errores de memoria}
\end{acronym}
...
A finales del siglo XIX, alrededor de 1880, surgen las videocámaras \cite{gray1993multiformat} con fines cinematográficos, y desde entonces son dispositivos que no han dejado de evolucionar tanto en forma como en calidad, funcionalidad o tamaño.

\hfill\break
A mitad del siglo XX, en 1942, nace en plena Segunda Guerra Mundial el término que se acuña como videovigilancia, de la mano del primer circuito de televisión cerrado o CCTV. Fue en este momento cuando se comenzó a utilizar un sistema videográfico \enquote*{a tiempo real} como elemento de seguridad en un entorno crítico \cite{fan2015survey}. Ver Figura \ref{fig:cctv_simple}.
...
Al principio, estos circuitos de televisión eran enormemente sencillos y solo incluían cámaras en blanco y negro conectadas a monitores remotos, que eran a su vez utilizados para controlar el lanzamiento de cohetes. Desde entonces y hasta la fecha, los avances en videovigilancia han tenido un enorme auge y han propiciado el desarrollo de nuevas tecnologías que han ido escalando a la vez que estos sistemas.
...
Poco después, en la década de 1960, se comenzó a popularizar el uso de cámaras de videovigilancia en lugares públicos, situándolas en puntos estratégicos que permitían aprovechar al máximo sus recursos e incluso pasar desapercibidas en muchos casos \cite{videovigilancia_historia}. 

\hfill \break
Unos años más tarde, este auge de la videovigilancia comienza a afectar a otros sectores de la sociedad y se populariza la utilización de CCTV en otros lugares, como bancos y tiendas, empleándose como una medida de seguridad añadida contra robos. Es gracias a este auge que, al igual que sucedió en el sector de la informática, las cámaras de videovigilancia fueron mejorando enormemente las características de los sistemas, abaratando costes, reduciendo sus tamaños y mejorando la visibilidad en situaciones de poca luz, lo que conllevó con el paso de los años a su irrupción en el mercado doméstico como medida de seguridad para disuadir a posibles intrusos.

\section{Visión Nocturna}

Los sistemas de videovigilancia críticos operan frecuentemente en condiciones de baja luminosidad, por lo que resulta pertinente mencionar brevemente las principales tecnologías de visión nocturna disponibles. No obstante, la validación del medidor de latencias con cámaras de visión nocturna o térmicas queda fuera del alcance del presente proyecto y se contempla como trabajo futuro.

Las tecnologías de visión nocturna pueden clasificarse en cuatro categorías principales \cite{chrzanowski2013review}:

\begin{itemize}
    \item \textbf{Visión nocturna analógica}: utiliza un tubo intensificador de imagen con fotocátodo y pantalla de fósforo para amplificar la luz ambiental residual. Clasificada en tres generaciones por el Ejército de EE.\,UU. \cite{kotulak1992methods}, desde los primeros \textit{Starlight Scopes} de la década de 1960 hasta los dispositivos actuales con fósforo blanco y arseniuro de gallio (GaAs).
    \item \textbf{Visión nocturna digital}: emplea sensores CMOS o CCD de alta sensibilidad que operan en el espectro NIR, con postprocesado digital de la imagen \cite{tsuji2002development}. Ofrecen menor coste y capacidad de superposición de información, a cambio de menor resolución que los analógicos.
    \item \textbf{Visión térmica}: detecta radiación infrarroja mediante microbolómetros cuya resistencia varía con la temperatura. No requiere luz ambiental, pero ofrece resoluciones inferiores y dificultad en la interpretación de escenas isotermas.
    \item \textbf{Iluminación infrarroja activa}: dispositivos que proyectan luz infrarroja invisible al ojo humano para iluminar escenas en oscuridad total, complementando a los anteriores \cite{sionyx_night_vision}.
\end{itemize}

Cada una de estas tecnologías introduce latencias de procesamiento específicas (conversión fotoeléctrica, digitalización, postprocesado) que podrían ser objeto de medición con el dispositivo desarrollado en este proyecto, una vez se disponga del hardware de visión nocturna necesario para las pruebas.

\section{Videovigilancia IP}

Se conoce como videovigilancia IP a aquella tecnología de seguridad que permite la observación mediante cámaras de vídeo pertenecientes a un CCTV en red, utilizando para ello Internet y protocolos de interconexión. Con ello se consigue la transmisión y supervisión remotas de vídeo a distancia aun sin estar conectado directamente al CCTV \cite{norris2004growth}.

Esta tecnología, de alta implantación y utilización en la actualidad, aprovecha el cableado de red local existente para la interconexión de cámaras del circuito e incluso permite transmitir las imágenes al exterior de la red local por medio de un ISP, posibilitando la transmisión de imágenes en línea. Con ello se ahorra parte del coste del cableado que utilizan los CCTV y se reaprovecha en muchos casos el ya existente de red. Otra posible ventaja que ofrecen es la completa eliminación de cables y la transmisión de vídeo mediante tecnologías inalámbricas \cite{ipvm}.
...
\section{Estado de la técnica en medición de latencia}

La medición de la latencia extremo a extremo (\textit{sensor-to-screen}) en sistemas de videovigilancia constituye un problema abierto que ha sido abordado desde distintas perspectivas en la literatura reciente.

\subsection{Definición del problema}

En el contexto de la videovigilancia en red, la latencia se define como el intervalo temporal transcurrido desde que un fotograma es captado por el sensor de la cámara hasta que ese mismo fotograma es presentado en el monitor del operador. Este pipeline incluye múltiples etapas: captura del sensor, codificación, transmisión por red, decodificación y renderizado en pantalla. Cada una de ellas contribuye de forma aditiva a la latencia total del sistema.

\subsection{Métodos basados en fotosensor y estímulo luminoso}

El enfoque más directo para medir la latencia extremo a extremo consiste en generar un estímulo luminoso controlado frente al objetivo de la cámara y detectar su aparición en el monitor mediante un fotosensor. La diferencia temporal entre la emisión del estímulo y la detección de su representación en pantalla constituye la latencia del sistema. Este es precisamente el principio de funcionamiento del dispositivo desarrollado en este proyecto.

Kaknjo et al. \cite{latency_measurement} propusieron un método similar utilizando un fotosensor y un LED para medir latencias en sistemas de teleoperación robótica, obteniendo valores típicos entre 488\,ms y 850\,ms en redes LAN, y entre 558\,ms y 1211\,ms a través de Internet. En un trabajo posterior \cite{latency_measurement2}, los mismos autores extendieron el método incorporando un FPGA como elemento de seguridad \textit{bump-in-the-wire}, demostrando que la adición de cifrado hardware incrementa la latencia en aproximadamente 33\,ms por fotograma.

\subsection{Métodos basados en marcas temporales y sincronización}

Una segunda familia de métodos se basa en la inserción de marcas temporales (\textit{timecodes}) en la señal de vídeo y su posterior lectura en el extremo receptor. Este enfoque requiere sincronización precisa entre emisor y receptor, típicamente mediante servidores NTP o PTP. Su principal limitación reside en la necesidad de modificar la señal de vídeo o disponer de acceso al software de la cámara, lo que no siempre es posible en instalaciones de videovigilancia operativas.

\subsection{Métodos basados en detección de objetos}

Trabajos recientes han explorado la utilización de técnicas de visión artificial para automatizar la medición de latencia. En particular, se ha propuesto el uso de modelos de detección de objetos (YOLO-V5) para identificar cambios en la escena y calcular el retardo de respuesta en sistemas de videovigilancia ferroviaria, eliminando la necesidad de hardware dedicado a cambio de una mayor complejidad computacional y menor precisión temporal.

\subsection{Influencia del medio de transmisión en la latencia}

Un aspecto frecuentemente sobreestimado en las instalaciones de videovigilancia es la contribución del cableado a la latencia total del sistema. La propagación de la señal eléctrica u óptica a través del medio físico introduce un retardo que es función directa de la longitud del enlace y de la velocidad de propagación del medio.

\subsubsection{Retardo de propagación por tipo de cable}

Según las especificaciones de la norma ANSI/TIA-568 \cite{tia568}, el retardo de propagación típico para cableado de cobre UTP categoría 5e es inferior a 5\,ns/m, con un valor máximo admisible de 5,7\,ns/m \cite{fluke_propagation}. Para un enlace Ethernet de longitud máxima (100\,m), esto supone un retardo de propagación de aproximadamente 500\,ns (0,5\,\textmu s).

En el caso de la fibra óptica monomodo, la velocidad de propagación de la luz en el vidrio es aproximadamente $c/n$, donde $n \approx 1{,}47$ es el índice de refracción típico. Esto se traduce en un retardo de aproximadamente 4,9\,\textmu s por kilómetro de fibra \cite{iosrjournals_fiber}. Para instalaciones de campus con distancias de 1--2\,km, el retardo de propagación pura se sitúa en el rango de 5--10\,\textmu s.

El cable coaxial (RG-59, RG-6), ampliamente utilizado en instalaciones CCTV analógicas, presenta velocidades de propagación entre 0,66c y 0,82c dependiendo del dieléctrico, resultando en retardos de 4--5\,ns/m, similares al UTP.

\subsubsection{Contribución relativa del cableado frente a otros factores}

Estos valores de retardo de propagación son órdenes de magnitud inferiores a las latencias introducidas por los demás componentes del pipeline de vídeo. Según el \textit{whitepaper} de Axis Communications \cite{axis_latency}, la latencia típica en un sistema de videovigilancia IP se descompone en:

\begin{itemize}
    \item \textbf{Captura y exposición del sensor}: 8--33\,ms (dependiendo del framerate).
    \item \textbf{Codificación H.264/H.265}: 30--100\,ms (según perfil y GOP).
    \item \textbf{Transmisión por red}: 1--5\,ms en LAN (dominado por buffering de switches, no por propagación).
    \item \textbf{Decodificación y renderizado}: 30--80\,ms.
\end{itemize}

La latencia total típica oscila entre 100\,ms y 300\,ms para sistemas IP optimizados, pudiendo superar 1\,s en configuraciones con múltiples saltos o codificación de alta compresión \cite{latency_measurement}.

\subsubsection{Impacto indirecto de la longitud del cable}

Aunque la propagación pura es despreciable, las tiradas de cable largas impactan indirectamente en la latencia al requerir equipos activos intermedios (switches, amplificadores, media converters). Cada switch gestionado en modo \textit{store-and-forward} puede añadir entre 1 y 4\,ms de latencia por salto, mientras que los amplificadores de señal coaxial introducen retardos del orden de decenas de microsegundos. En sistemas con múltiples saltos de red, la latencia acumulada de los equipos intermedios puede alcanzar valores significativos.

Adicionalmente, un cableado degradado o de categoría inferior provoca retransmisiones a nivel de enlace en sistemas digitales, incrementando la latencia efectiva de forma impredecible.

\subsection{Posicionamiento del presente proyecto}

El dispositivo desarrollado en este proyecto se enmarca en la primera categoría (fotosensor + estímulo luminoso), con las siguientes diferencias respecto al estado de la técnica:

\begin{itemize}
    \item \textbf{Portabilidad}: a diferencia de los montajes de laboratorio de Kaknjo et al., el medidor está diseñado como un dispositivo autónomo y transportable, alimentado por batería.
    \item \textbf{Interfaz integrada}: incorpora una pantalla táctil con interfaz gráfica Qt que permite al técnico operar el dispositivo sin necesidad de un ordenador externo.
    \item \textbf{Independencia del sistema}: no requiere modificar la señal de vídeo, acceder al software de la cámara ni sincronizar relojes entre dispositivos. El método es puramente óptico y funciona con cualquier cámara y monitor.
    \item \textbf{Bajo coste}: construido íntegramente con componentes de hardware abierto (Raspberry Pi, ADS1115, fotosensor genérico, LED), frente a soluciones que emplean FPGAs o equipamiento de laboratorio especializado.
\end{itemize}
...
La principal función de este equipo medidor de latencias es la de cuantificar el retardo de un sistema de videovigilancia tomando como punto inicial la toma de la imagen y como final, su retransmisión a través del monitor, mostrando este cálculo el equipo en su pantalla.
El dispositivo podrá así mismo graficar el retardo que tiene la entrada en ser procesada y mostrada con respecto a una línea temporal. El principio de medición es aplicable a cualquier sistema que produzca una imagen visible en un monitor, si bien la validación con cámaras de visión nocturna o térmicas requerirá un ajuste del umbral de detección del fotosensor que se contempla como trabajo futuro.
...
\begin{itemize}
    \item \textbf{Capacidad de medición en entornos muy luminosos}. Se tiene dependencia de un LED que puede variar en tamaño e intensidad, pero que en situaciones de mucha luz se verá comprometido para conseguir destacar en el ambiente. Se considera en este proyecto un ambiente muy luminoso cualquiera que supere los 300 lúmenes, como puedan ser estancias con amplia iluminación natural o exposición solar directa. %TODO: realizar medidas y variar el número en consonancia
    ...
    \item \textbf{Capacidad de la batería}. Factor limitante en tanto en cuanto se depende de ella para el funcionamiento del dispositivo y establece cuánto tiempo podrá permanecer en funcionamiento el equipo de medición. A su vez, la restricción a la batería es impuesta por el coste de la misma, el peso que añada al dispositivo, que debe seguir siendo portátil, y el propio espacio disponible para ella dentro de la carcasa con el resto de componentes ensamblados.
    \item \textbf{Longitud de los cables del medidor}. Será una restricción debido a que no se podrán realizar mediciones en CCTV que ya se encuentren instalados, ya que el LED y el fotosensor tienen únicamente un metro de separación entre ellos. Esto será debido a que, en primera instancia, el diseño del dispositivo no contempla la latencia introducida por su propio cableado en el tiempo de encendido del LED y la recepción de las señales del sensor, lo cual obliga a utilizar un cable de corta longitud, de un metro, para no añadir más variables a las mediciones.
    \item \textbf{Longitud de los cables de los CCTV}. Se precisarán los cables que se utilizarán en la instalación de los equipos para llevar a cabo las mediciones, ya que dos cámaras conectadas al mismo monitor, pero con cables de longitud distinta, tendrán distinta latencia.
\end{itemize}
...
\begin{itemize}
    ...
    \item Biblioteca de funciones estándar de C++.
    \item Biblioteca de funciones boost \cite{boost}. En su versión 1.77, que proporciona funcionalidad adicional sobre la biblioteca estándar de C++, permitiendo aprovechar desarrollos de algoritmos ya implementados y probados en lugar de desarrollarlos desde cero para este proyecto.
    \item Qt framework \cite{qt}. En su versión 6, el cual servirá para el desarrollo de la interfaz de usuario del equipo de medición y ayudar a elevar el nivel de la implementación gracias a la simplificación que provee para tareas que de otro modo serían muy tediosas. Incluye además el módulo Qt Virtual Keyboard para entrada táctil sin dependencias de teclados virtuales externos.
    \item Sistema de construcción qmake \cite{qt}, incluido con Qt, que genera los ficheros Makefile a partir del archivo de proyecto (.pro) y gestiona automáticamente las dependencias, la invocación del preprocesador de meta-objetos (MOC), la compilación de formularios (.ui) y la generación de traducciones (.qm).
    \item GNU Make \cite{gnumake}, herramienta de automatización de compilación que ejecuta los Makefile generados por qmake, resolviendo el grafo de dependencias entre ficheros fuente y objetos, y paralelizando la compilación cuando es posible.
    \item ARM GNU Toolchain \cite{armtoolchain} (anteriormente Linaro), cadena de herramientas de compilación cruzada que permite generar binarios ejecutables para arquitectura ARM v8 (Raspberry Pi) desde un ordenador de desarrollo x86-64. Incluye los compiladores \texttt{aarch64-linux-gnu-gcc} y \texttt{aarch64-linux-gnu-g++}, el enlazador y las utilidades binarias asociadas.
    \item Se utilizarán algunas bibliotecas de funciones externas extraídas de Internet para facilitar el desarrollo de ciertas funcionalidades. Algunas de ellas serán: QCustomPlot \cite{qtcustomplot}, para graficar los resultados de las mediciones, pigpio \cite{pigpio} para facilitar el acceso a los pines GPIO de la Raspberry Pi, rpi\_ads1115 \cite{rpi_ads1115} para la lectura analógica del fotosensor a través del conversor analógico-digital ADS1115 mediante I2C, y libgpiod \cite{libgpiod} para la gestión de interrupciones GPIO mediante la interfaz de dispositivo de caracteres del kernel Linux.
    \item Doxygen \cite{doxygen}, herramienta de generación automática de documentación a partir de los comentarios presentes en el código fuente C++. Produce documentación navegable en formato HTML con diagramas de clases, grafos de llamadas y relaciones de herencia, facilitando la comprensión y mantenimiento del proyecto.
    \item Graphviz \cite{graphviz} (dot), motor de renderizado de grafos utilizado por Doxygen para generar diagramas de clases, colaboración y llamadas en formato SVG. Permite visualizar la arquitectura del software de forma automática a partir del análisis estático del código.
    ...
    \item AddressSanitizer \cite{asan} (ASan), herramienta de detección de errores de memoria integrada en GCC y Clang. Se activa automáticamente en compilaciones Debug sobre Linux, y detecta en tiempo de ejecución fugas de memoria, desbordamientos de buffer y accesos a memoria liberada, abortando el proceso con un informe detallado del lugar exacto del error.
    \item Docker \cite{docker}, plataforma de contenedores que permite empaquetar y ejecutar aplicaciones en entornos aislados y reproducibles. En este proyecto se utiliza para crear un contenedor ARM64 (Ubuntu 24.04) que emula el entorno de ejecución de la Raspberry Pi mediante QEMU \cite{qemu} user-mode, permitiendo validar el binario de producción sin hardware físico. Adicionalmente, un Dockerfile multi-stage encapsula toda la cadena de compilación (Qt, libgpiod, pigpio, compiladores cruzados) para generar el binario de forma completamente portable desde cualquier máquina con Docker instalado.
    \item Valgrind \cite{valgrind} (memcheck), herramienta de análisis dinámico de memoria que se ejecuta sobre el binario ARM64 dentro del contenedor Docker. Detecta fugas de memoria, accesos a memoria no inicializada y lecturas/escrituras fuera de límites sin necesidad de recompilar el código. Se utiliza como complemento de ASan para el entorno ARM64, donde AddressSanitizer no es compatible con la emulación QEMU.
\end{itemize}
...
Como se detalló en la comparativa técnica de la sección \ref{hardware} (tabla \ref{tab:hardware}), se ha elegido la Raspberry Pi 3 Model B frente a Arduino Uno debido a su capacidad para ejecutar un sistema operativo completo, soportar pantallas táctiles de alta resolución y ofrecer conectividad de red integrada — requisitos fundamentales para la interfaz gráfica Qt del medidor de latencias.
...
A estas herramientas habrá que añadir una base de trabajo en forma de sistema operativo, que para el caso será Raspberry Pi OS (anteriormente conocida como Raspbian), versión Lite, distribución Linux basada en Debian para Raspberry Pi y que por antonomasia es referente en buen funcionamiento y estabilidad. Este sistema irá personalizado para ofrecer una experiencia únicamente centrada en la medición de latencias.
Sobre Raspberry Pi OS se ejecutará un script a la inicialización del sistema que será el encargado de lanzar la aplicación automáticamente.
...
Para facilitar todo el desarrollo en un entorno controlado, se pretendía, en un inicio, utilizar una máquina virtual con Raspberry Pi OS que correría sobre VirtualBox \cite{virtualbox}. Se encontró el impedimento de que no existe una versión de Raspberry Pi OS suficientemente actualizada para la ejecución del proyecto en arquitectura x86-64. A esto hay que añadir la incompatibilidad de las bibliotecas de funciones que, compiladas en arquitectura x86-64, no resultarían útiles a la hora de portar el proyecto software a la placa programable final. Además, al ser un proyecto que trabaja directamente sobre el hardware de la placa (pines GPIO y pantalla táctil), se imposibilitaría la realización de pruebas sobre la máquina virtual, requiriendo del hardware real para poder verificar todo el código relacionado con estos aspectos.
Se propone en este caso trabajar directamente sobre arquitectura x86, sin necesidad de virtualizar, con el objetivo de aumentar el rendimiento de trabajo, acortar tiempos de espera y simplificar la sincronización del hardware ARM con la máquina x86, eliminando la capa hipervisora que de otra forma estaría funcionando sobre el ordenador de desarrollo.
A este respecto se ha utilizado una compilación cruzada a través de un compilador para ARM que trabaja directamente sobre las bibliotecas de funciones extraídas de la Raspberry Pi, ya precompiladas en ARM. Todos estos ficheros se extraen y copian a la máquina x86, la cual servirá de plataforma para la compilación cruzada y el despliegue del proyecto posterior en la placa programable.
La interconexión para compartir las bibliotecas de funciones entre la Raspberry Pi y el PC x86 se realizará a través de conexión SSH \cite{ssh}, mediante la herramienta rsync \cite{rsync}. A su vez, a la hora de desplegar la aplicación ya compilada desde la máquina x86, se volverá a recurrir al protocolo SSH \cite{ssh}.
Para compilar el proyecto se utilizará el ARM GNU Toolchain, antiguamente conocido como Linaro. La configuración de la toolchain necesaria para compilar desde un ordenador x64 a ARM v8 se explicará más adelante en este documento, pero se adelanta que funcionará utilizando el compilador gcc y g++ en sus respectivas ediciones ARM.
...
Una consideración importante durante el análisis de la placa a utilizar es que la Raspberry, frente a Arduino, no tiene pines de entrada/salida analógicos, con lo cual no podrá ser utilizada inicialmente para hacer lecturas del sensor de luz OPT101, que es, en este caso, el elegido para el proyecto frente al resto de sus competidores de tipo fotorresistivo, los cuales tienen un tiempo de respuesta superior y no son tan apropiados para hacer mediciones de latencias que deben ser prácticamente instantáneas. Es este y no otro el motivo por el cual ha sido elegido candidato para la realización de este proyecto. Para salvar este escollo, que no se tuvo en cuenta durante el análisis inicial del proyecto, se ha recurrido a I2C \cite{i2c}, una interfaz de intercomunicación de circuitos que tiene integrada la Raspberry Pi en sus pines GPIO y que es posible activar desde la configuración del sistema. Será utilizando esta interfaz, la biblioteca pigpio \cite{pigpio} y un conversor de señal analógico-digital modelo ADS1115.

\chapter{Desarrollo del proyecto}

El medidor de latencias para sistemas de videovigilancia es un dispositivo que, de manera sencilla, a través de la emisión y captación de un impulso lumínico, realiza los cálculos necesarios para averiguar el tiempo que necesita un dispositivo de vídeo desde que capta una imagen hasta que la proyecta en un monitor.
Se detallarán en este capítulo las fases de su ciclo comercial, cómo funciona, qué características y apariencia tendrá su sistema software, qué problemas es capaz de resolver y cómo está programado.

\section{Modelo de ciclo de vida}

Se describe a continuación cuáles son las posibilidades del producto a lo largo de su vida útil más allá del alcance de este proyecto. Dado que se pretende comercializar el producto, se proporciona a continuación un modelo de ciclo de vida con la planificación estratificada de las expectativas actuales al respecto.
...
\begin{enumerate}
    ...
    \item Como usuario se desea que su aspecto físico sea el de una caja plástica PLA rectangular, impresa en 3D con unas dimensiones de (insertar dimensiones reales aquí), que incorpore una pantalla táctil en su frontal y un conmutador de encendido en alguno de sus laterales.
...
\end{enumerate}
    ...
    \label{rf16}
    \item Como usuario se desea que desde la pantalla de mediciones se puedan visualizar los resultados de las mediciones de latencia en forma de gráfico bidimensional cíclico, con base en el tiempo entre mediciones especificado, en tiempo real mientras se van realizando. 
    \item Como usuario se quiere mostrar numéricamente en ms la latencia del sistema medido en tiempo real mientras se realiza la medición.
...
Comentarios               & Se podrá borrar también la medición una \\
                          & vez consultados los datos de esta desde la \\
...
En primer lugar, se procede a la preparación de la pantalla. Se le quitan los separadores que unen la PCB a la pantalla propiamente dicha y se atornilla la pantalla completa a la parte superior de la carcasa impresa previamente en 3D.
...
Para finalizar la instalación se instalará la tarjeta micro SD con el software previamente cargado en su ranura correspondiente.

* Para el prototipo se ha fabricado una nueva versión de la carcasa utilizando como molde la original modificada a mano anteriormente, y complementada con los componentes que posteriormente se instalarán en ella para la correcta medición de los espacios necesarios para los puertos de carga y la instalación del controlador de las baterías.
...
Adicionalmente se emplearon las siguientes herramientas de soporte:
\begin{itemize}
    \item \textbf{Docker} con emulación ARM64 (qemu-user-static): permite ejecutar y validar el binario ARM64 sobre cualquier máquina x86-64 sin hardware Raspberry Pi, proporcionando un entorno gráfico emulado vía VNC a 800$\times$480.
    \item \textbf{Valgrind} (memcheck): análisis dinámico de memoria sobre el binario ARM64 dentro del contenedor Docker, detectando fugas, accesos inválidos y uso de memoria no inicializada.
    \item \textbf{AddressSanitizer} (ASan): instrumentación de compilación para detección de errores de memoria en tiempo de ejecución sobre Desktop Linux.
    \item \textbf{Doxygen} con \textbf{Graphviz}: generación automática de documentación del código fuente (referencia de API, diagramas de clases y grafos de dependencias).
\end{itemize}
...
\begin{itemize}
    ...
    \item Las imágenes recientes de Raspberry Pi OS utilizan sectores de 4096 bytes en lugar de 512, generando warnings de \texttt{fdisk} y offsets incorrectos si no se tiene en cuenta.
    ...
    \item La máquina \texttt{raspi3b} de QEMU es altamente inestable con kernels recientes: el proceso QEMU finalizaba inmediatamente sin mostrar salida ni error, al no ser compatible el \texttt{kernel8.img} de imágenes Raspberry Pi OS modernas con la emulación limitada del hardware (GPU VideoCore no emulada).
...
QTest & Windows + Linux & Pruebas unitarias automatizadas (145 superados, 2 pendientes de hardware) \\ \hline
...
A continuación se presentan los resultados obtenidos tras la ejecución completa de la batería de pruebas en los dos entornos de validación disponibles: Desktop (x86\_64) y ARM64 virtualizado (Docker con QEMU user-mode).

\subsubsection{Entorno Desktop (Windows x86\_64)}

La ejecución de los 147 tests unitarios sobre la plataforma de desarrollo principal (Windows 10/11, MinGW 13.1.0 x86\_64, Qt 6.11.1) arroja los siguientes resultados:

\begin{table}[H]
\centering
\caption{Resultados de pruebas unitarias --- Desktop (Windows x86\_64)}
\label{tab:results_desktop}
\begin{tabular}{|p{4.5cm}|c|c|c|c|}
\hline
\textbf{Suite} & \textbf{Tests} & \textbf{Pass} & \textbf{Fail} & \textbf{Skip} \\ \hline
tst\_datamodel & 17 & 17 & 0 & 0 \\ \hline
tst\_appsettings & 23 & 23 & 0 & 0 \\ \hline
tst\_jsonoperator & 14 & 14 & 0 & 0 \\ \hline
tst\_sensoroperator & 14 & 12 & 0 & 2 \\ \hline
tst\_homescreen & 10 & 10 & 0 & 0 \\ \hline
tst\_measurescreen & 14 & 14 & 0 & 0 \\ \hline
tst\_settingsscreen & 20 & 20 & 0 & 0 \\ \hline
tst\_helpscreen & 11 & 11 & 0 & 0 \\ \hline
tst\_registryscreen & 10 & 10 & 0 & 0 \\ \hline
tst\_registrydisplayscreen & 14 & 14 & 0 & 0 \\ \hline
\textbf{TOTAL} & \textbf{147} & \textbf{145} & \textbf{0} & \textbf{2} \\ \hline
\end{tabular}
\end{table}

Los 2 tests omitidos (\textit{skipped}) corresponden a escenarios de timeout del sensor que no pueden reproducirse con los stubs de hardware, ya que estos devuelven valores inmediatamente sin simular retardo de respuesta. Estos escenarios se validarían exclusivamente sobre el hardware real.

La compilación en modo Debug no produce warnings ni errores (\texttt{0 warnings, 0 errors}) tanto para el proyecto principal como para el sub-proyecto de tests.

\subsubsection{Entorno Desktop (Linux x86\_64) con AddressSanitizer}

La ejecución sobre Linux (Ubuntu 24.04, GCC 13, Qt 6.11.1) con AddressSanitizer habilitado produce los mismos resultados: 145 tests superados y 2 omitidos. El informe de ASan al finalizar indica:

\begin{itemize}
    \item \textbf{Fugas reales detectadas}: 0 (tras la corrección del bug en \texttt{JsonOperator}).
    \item \textbf{Fugas reportadas (falsos positivos)}: $\sim$19 KB atribuidos a la caché interna de fontconfig y $\sim$3 KB a pools de memoria internos de Qt. Ambos se liberan al terminar el proceso y no representan fugas reales.
\end{itemize}

El bug original detectado por ASan consistía en una fuga de 448 bytes por cada llamada a \texttt{loadFileFromDisk()} y \texttt{saveMeasureToDisk()} en la clase \texttt{JsonOperator}, donde se realizaba \texttt{new QFile(...)} sin liberar la instancia anterior. Se corrigió añadiendo \texttt{delete mFile;} antes de cada nueva asignación.

\subsubsection{Entorno ARM64 virtualizado (Docker + QEMU)}

Para la validación sobre la plataforma destino, se ejecutó el binario ARM64 compilado con GCC 13 (\texttt{aarch64-linux-gnu-g++-13}) en modo stubs (\texttt{USE\_STUBS=1}) dentro del contenedor Docker Ubuntu 24.04 ARM64 emulado vía \texttt{qemu-user-static}.

\begin{table}[H]
\centering
\caption{Resultados de pruebas unitarias --- ARM64 virtualizado (Docker)}
\label{tab:results_arm64}
\begin{tabular}{|p{4.5cm}|c|c|c|c|}
\hline
\textbf{Suite} & \textbf{Tests} & \textbf{Pass} & \textbf{Fail} & \textbf{Skip} \\ \hline
tst\_datamodel & 17 & 17 & 0 & 0 \\ \hline
...
Los resultados son idénticos a los del entorno Desktop, confirmando que la compilación cruzada produce un binario funcionalmente equivalente. La ejecución se realiza en modo \texttt{offscreen} (\texttt{-platform offscreen}) para no requerir display gráfico.

\subsubsection{Análisis de memoria ARM64 con Valgrind}

Se ejecutó Valgrind con la herramienta \texttt{memcheck} sobre el binario ARM64 dentro del contenedor Docker:

\begin{verbatim}
valgrind --leak-check=full --show-leak-kinds=definite \
  ./LatencyTester -platform offscreen
\end{verbatim}

\textbf{Resultado}:
\begin{itemize}
    \item \textbf{Fugas definidas (\textit{definitely lost})}: 0 bytes en 0 bloques.
    \item \textbf{Fugas indirectas (\textit{indirectly lost})}: 0 bytes en 0 bloques.
    \item \textbf{Fugas posibles (\textit{possibly lost})}: $\sim$2 KB atribuidos a internos de Qt (thread pools) --- comportamiento normal y documentado.
    \item \textbf{Accesos inválidos (\textit{invalid read/write})}: 0.
    \item \textbf{Uso de memoria no inicializada}: 0.
\end{itemize}

Valgrind se utiliza como complemento de ASan para el entorno ARM64, ya que AddressSanitizer no es compatible con la emulación QEMU user-mode y por tanto no puede activarse en compilaciones cruzadas.

\subsubsection{Validación gráfica vía VNC}

Se realizó una validación visual de la interfaz gráfica ejecutando el binario ARM64 dentro del contenedor Docker con el servidor VNC activo (Xvfb 800$\times$480 + x11vnc en puerto 5900). Se verificó:

\begin{itemize}
    \item Renderizado correcto de todas las pantallas (Home, Medición, Ajustes, Ayuda, Registro, Visualización de resultados).
    \item Navegación entre pantallas mediante el botón \textit{Atrás} y los botones del menú principal.
    \item Aplicación de temas (modo claro y modo oscuro) con transición correcta de estilos.
    \item Modo daltónico con prioridad sobre el modo oscuro (colores de alto contraste visibles).
    \item Sliders de configuración con actualización en tiempo real de las etiquetas numéricas.
    \item Gráfica QCustomPlot renderizada correctamente con datos de prueba.
    \item Teclado virtual Qt funcional para renombrar mediciones.
    \item Dimensiones de pantalla 800$\times$480 respetadas (sin desbordamiento ni recorte de widgets).
\end{itemize}

La resolución coincide con la pantalla táctil de 7 pulgadas del dispositivo real, proporcionando una simulación visual fiel del entorno de producción.

\subsubsection{Resumen de resultados}

\begin{table}[H]
\centering
\caption{Resumen consolidado de validación}
\label{tab:results_summary}
\begin{tabular}{|p{3.5cm}|p{3.5cm}|p{2.5cm}|p{3.5cm}|}
\hline
\textbf{Entorno} & \textbf{Herramienta} & \textbf{Resultado} & \textbf{Observaciones} \\ \hline
Windows x86\_64 & QTest & 145/147 pass & 2 skip (timeout stubs) \\ \hline
Linux x86\_64 & QTest + ASan & 145/147 pass & 0 fugas reales \\ \hline
ARM64 Docker & QTest (offscreen) & 145/147 pass & 2 skip (timeout stubs) \\ \hline
ARM64 Docker & Valgrind memcheck & 0 fugas & 0 accesos inválidos \\ \hline
ARM64 Docker & VNC (visual) & Correcto & Todas las pantallas OK \\ \hline
\end{tabular}
\end{table}

Los resultados demuestran que:
\begin{enumerate}
    \item El software es funcionalmente correcto en ambas arquitecturas (x86\_64 y ARM64), con idéntico comportamiento lógico independientemente de la plataforma de ejecución.
    \item No existen fugas de memoria ni accesos a memoria inválida en ninguno de los entornos validados.
    \item La interfaz gráfica se renderiza correctamente sobre la resolución de 800$\times$480 del dispositivo destino, incluyendo todos los modos de visualización (estándar, nocturno y daltónico).
    \item La compilación cruzada con GCC 13 y Qt 6.11.1 ARM64 produce binarios ejecutables que funcionan de forma idéntica a la compilación nativa, validando la corrección del toolchain configurado.
\end{enumerate}

Los únicos tests omitidos (2 de 147) corresponden a escenarios de timeout del sensor que requieren hardware real para su verificación, al no poder simularse retardos de respuesta con los stubs. Estos se validarán durante la fase de integración con la Raspberry Pi física.

\chapter{Resumen}

El presente proyecto ha desarrollado un sistema software embebido para la medición de latencias en sistemas de videovigilancia críticos, ejecutable sobre una plataforma Raspberry Pi 3 Model B equipada con pantalla táctil de 7 pulgadas. El dispositivo permite cuantificar de forma objetiva y repetible el retardo de vídeo introducido por las cadenas de transmisión CCTV, un parámetro crítico en entornos de seguridad donde el operador debe reaccionar en tiempo real ante eventos captados por las cámaras.

\section*{Desarrollo realizado}

El software se ha implementado en C++20 sobre el framework Qt 6.11.1, empleando una arquitectura modular que separa la lógica de negocio (\textit{Core}) de la interfaz gráfica (\textit{GUI}). La aplicación consta de siete pantallas independientes gestionadas mediante un \texttt{QStackedWidget}, cubriendo las funcionalidades de medición, calibración, configuración, registro histórico y ayuda integrada.

El principio de medición se basa en la emisión de un pulso luminoso mediante un LED controlado por GPIO y la detección simultánea de dicho pulso por un fotosensor ADS1115 conectado vía I2C. La diferencia temporal entre la emisión y la detección en la imagen visualizada constituye la latencia medida. El software gestiona de forma autónoma los ciclos de calibración, la secuencia de mediciones configurables por el usuario, el cálculo estadístico y la persistencia de resultados en formato JSON.

Se han implementado características de accesibilidad (modo daltónico con colores de alto contraste, modo nocturno con esquema oscuro, tamaño de fuente configurable) e internacionalización (español, inglés y polaco), con traducciones embebidas en el binario mediante el sistema \texttt{embed\_translations} de Qt 6.

\section*{Infraestructura de compilación y despliegue}

Uno de los logros técnicos más significativos del proyecto ha sido la configuración de una toolchain de compilación cruzada completa para ARM64. Esta infraestructura permite generar el binario de producción desde una estación de trabajo x86\_64 sin necesidad de compilar directamente en la Raspberry Pi, cuya capacidad de cómputo y memoria resultan insuficientes para manejar la compilación de un proyecto Qt de esta envergadura.

La cadena de herramientas final incluye:
\begin{itemize}
    \item Compilador cruzado GCC 13 (\texttt{aarch64-linux-gnu-g++-13}).
    \item Qt 6.11.1 compilado desde código fuente para ARM64, instalado en \texttt{/opt/Qt/6.11.1/arm64/}.
    \item Sysroot ARM64 extraído de un contenedor Docker Ubuntu 24.04.
    \item Enlazador \texttt{lld} para evitar incompatibilidades de versiones de GLIBC.
\end{itemize}

El proceso de establecimiento de esta toolchain resultó ser la tarea de mayor complejidad técnica del proyecto, requiriendo la resolución iterativa de problemas de compatibilidad entre versiones de compilador, librería estándar, sistema operativo base y herramientas de Qt.

\section*{Validación}

Se estableció una estrategia de pruebas en tres niveles:
\begin{enumerate}
    \item \textbf{Pruebas unitarias} (QTest): 147 tests automatizados distribuidos en 10 suites, cubriendo la totalidad de los módulos Core y GUI. De estos, 145 se superan satisfactoriamente en los tres entornos de compilación (Windows x86\_64, Linux x86\_64, ARM64 emulado) y 2 quedan pendientes de validación con hardware real (escenarios de timeout del sensor).
    \item \textbf{Análisis de memoria} (AddressSanitizer + Valgrind): cero fugas de memoria reales y cero accesos inválidos detectados tras la corrección de los defectos encontrados durante la fase de desarrollo.
    \item \textbf{Validación sobre plataforma destino}: ejecución del binario ARM64 en un contenedor Docker con emulación QEMU user-mode, verificando el correcto funcionamiento gráfico sobre una resolución de 800$\times$480 mediante VNC.
\end{enumerate}
...
\begin{itemize}
    \item Manual de usuario integrado en la aplicación (HTML embebido accesible desde la pantalla de ayuda en los tres idiomas soportados).
    \item Manual del desarrollador con descripción de la arquitectura, APIs, procedimientos de compilación, testing, internacionalización y despliegue.
    \item Documentación del código fuente generada automáticamente con Doxygen (diagramas de clases, grafos de dependencias, referencia de API).
    \item Scripts de automatización para compilación, generación de documentación, gestión del contenedor Docker y compilación cruzada ARM64.
\end{itemize}

\section*{Estado actual}

El software se encuentra completamente funcional en modo de pruebas (stubs de hardware), habiéndose validado la corrección del código, la ausencia de fugas de memoria y el comportamiento gráfico sobre la resolución de destino. Queda pendiente exclusivamente la integración con los drivers de hardware reales (pigpio y libgpiod v2) para la generación del binario de producción final y su validación sobre la Raspberry Pi física.

\chapter{Conclusiones y trabajo futuro}

\section{Conclusiones}

Este proyecto nace con la ambición de desarrollar un producto de ingeniería completo: un dispositivo hardware portátil, con carcasa impresa en 3D, electrónica de adquisición, software embebido y sistema de alimentación autónomo. La realidad del desarrollo demostró rápidamente que el componente software, por sí solo, constituía un proyecto de envergadura suficiente para absorber toda la capacidad de trabajo disponible. Lo que inicialmente se concibió como una de las piezas del conjunto se convirtió en el eje central del proyecto.

La decisión de acotar el alcance al desarrollo software no fue trivial, pero resultó acertada. El medidor de latencias como concepto requiere un software fiable, mantenible y verificable, y alcanzar ese nivel de calidad exige un esfuerzo que no puede comprimirse para dejar espacio al diseño electrónico y mecánico dentro del mismo marco temporal. Se ha preferido entregar un producto software completo y profesional antes que un prototipo integral con deficiencias en cada una de sus capas.

Dentro del desarrollo software, la configuración de la toolchain de compilación cruzada resultó ser, con diferencia, el obstáculo técnico de mayor magnitud. Lo que en la literatura y documentación de Qt se describe como un proceso relativamente directo (instalar compilador cruzado, apuntar al sysroot, compilar) se transformó en una cadena de problemas interdependientes que consumieron una parte significativa del tiempo del proyecto:

\begin{itemize}
    \item La incompatibilidad de qmake con QEMU user-mode, que impedía ejecutar la herramienta de build dentro del contenedor emulado, obligando a rediseñar el flujo de compilación.
    \item La ausencia de Qt 6 precompilado para ARM64, que exigió compilar el framework completo desde código fuente — un proceso de más de dos horas con más de una decena de dependencias que resolver iterativamente.
    \item Los desajustes de versión de GLIBC entre el linker del host y las librerías del sysroot, solucionados mediante el linker alternativo \texttt{lld}.
    \item La necesidad de GCC 13 para obtener soporte completo de la cabecera \texttt{<format>} de C++20, lo que precipitó la actualización del sistema operativo de desarrollo de Ubuntu 22.04 a 24.04.
    \item La incompatibilidad de APIs entre libgpiod v1 (disponible en repositorios) y v2 (requerida por el driver ADS1115), que obligó a compilar la librería desde fuente con los bindings C++.
    \item La ausencia total de pigpio en los repositorios de Debian/Ubuntu para ARM64, requiriendo igualmente su compilación cruzada desde fuente.
\end{itemize}

Cada uno de estos problemas, de forma individual, tiene una solución relativamente sencilla. La dificultad radica en que se presentan encadenados: resolver uno revela el siguiente, y la documentación disponible asume configuraciones estándar que no se dan en un proyecto con esta combinación concreta de tecnologías (Qt 6.11 + ARM64 + C++20 + libgpiod v2 + pigpio + compilación cruzada desde x86-64).

Una vez resuelta la configuración de la toolchain, el resultado es un flujo de trabajo robusto y completamente automatizado. Un único script (\texttt{setup-toolchain.sh}) permite a cualquier desarrollador, partiendo de un sistema Ubuntu 24.04 limpio, obtener un binario de producción ARM64 sin intervención manual. Alternativamente, un contenedor Docker multi-stage (\texttt{Dockerfile.production}) encapsula absolutamente todo el entorno de compilación, permitiendo generar el binario desde cualquier máquina con Docker instalado — incluyendo Windows con WSL2.

El software desarrollado aborda un problema real en el ámbito de la videovigilancia para sistemas críticos. En entornos donde la respuesta del operador depende directamente de la latencia del sistema de vídeo — salas de control de infraestructuras críticas, centros de vigilancia penitenciaria, control fronterizo — no se ha identificado hasta ahora un instrumento portátil, económico y autónomo diseñado específicamente para cuantificar ese retardo. El dispositivo propuesto ofrece una solución que combina hardware de bajo coste (Raspberry Pi, LED, fotosensor ADC) con un software embebido que proporciona:

\begin{itemize}
    \item Medición automatizada con calibración previa para adaptarse a las condiciones ambientales.
    \item Interfaz táctil accesible, con soporte multilingüe y modos de visualización adaptados (nocturno, daltónico).
    \item Persistencia y consulta del historial de mediciones, facilitando la comparación temporal y la auditoría de sistemas.
    \item Verificación exhaustiva del software mediante 145 pruebas unitarias superadas (2 pendientes de validación con hardware real), análisis de memoria con AddressSanitizer y Valgrind, y validación gráfica sobre la resolución del dispositivo destino.
\end{itemize}

Cabe señalar que la precisión y repetibilidad de las mediciones deberá ser caracterizada mediante calibración frente a equipos de referencia una vez se disponga de la integración completa con el hardware real, aspecto que se contempla como trabajo futuro.

El proyecto demuestra que es viable desarrollar software embebido de calidad profesional sobre Qt 6 para plataformas ARM64, incluso partiendo de una cadena de herramientas que no está oficialmente soportada por el fabricante del framework. El coste de esa viabilidad es una inversión inicial considerable en la configuración del entorno, pero una vez amortizada, el ciclo de desarrollo se equipara al de cualquier aplicación Desktop convencional.

\section{Trabajo futuro}

El proyecto, en su estado actual, tiene el software completamente funcional y validado en entorno emulado. Las líneas de trabajo futuro se articulan en torno a tres ejes:

\subsection{Integración con hardware real}

\begin{itemize}
    \item \textbf{Validación sobre Raspberry Pi física}: desplegar el binario de producción y ejecutar la secuencia completa de calibración y medición con el LED y el fotosensor ADS1115 conectados. Los 2 tests unitarios actualmente omitidos (escenarios de timeout del sensor) se podrán validar en este entorno.
    \item \textbf{Ajuste de parámetros de medición}: los umbrales de detección del fotosensor, el tiempo de estabilización del LED y los timeouts pueden requerir ajuste fino con el hardware real, en función de las condiciones de iluminación y la distancia al monitor bajo prueba.
    \item \textbf{Calibración de precisión}: comparar las mediciones del dispositivo con equipos de referencia de laboratorio para establecer la incertidumbre del instrumento y, si procede, aplicar factores de corrección.
\end{itemize}

\subsection{Mejoras de software}

\begin{itemize}
    \item \textbf{Exportación de resultados}: añadir la posibilidad de exportar las mediciones a formatos CSV o PDF para facilitar la integración con informes de auditoría.
    \item \textbf{Modo de medición continua}: implementar un modo de monitorización que realice mediciones periódicas durante un intervalo prolongado (horas/días), generando un perfil temporal de la latencia del sistema bajo prueba.
    \item \textbf{Conectividad de red}: añadir un módulo de comunicación (WiFi o Ethernet) para enviar resultados a un servidor central, permitiendo la gestión remota de múltiples dispositivos desplegados en distintas localizaciones.
    \item \textbf{Actualización OTA}: implementar un mecanismo de actualización del firmware por red, evitando la necesidad de acceso físico al dispositivo para instalar nuevas versiones.
    \item \textbf{Actualización de QCustomPlot}: cuando la versión oficial sea compatible con Qt 6.8+ sin parches, eliminar la macro \texttt{QT\_NO\_CONSTEXPR\_METAOBJECT\_DATA} y las modificaciones al namespace QCP.
\end{itemize}

\subsection{Hardware y producto final}

\begin{itemize}
    \item \textbf{Diseño de PCB dedicada}: sustituir la combinación Raspberry Pi + placa de prototipos por una PCB diseñada a medida que integre el microcontrolador, el ADC, el driver del LED y el conector de pantalla en un único circuito.
    \item \textbf{Carcasa de producción}: evolucionar del prototipo impreso en PLA a una carcasa inyectada en ABS con certificación IP20 para uso industrial.
    \item \textbf{Certificación y homologación}: obtener el marcado CE para la comercialización del producto como instrumento de medida en el ámbito europeo.
    \item \textbf{Migración a plataforma más moderna}: evaluar la viabilidad de portar el software a un Raspberry Pi 4 o Compute Module 4, que ofrecen mayor potencia de cómputo y conectividad integrada, o a plataformas alternativas como los módulos NVIDIA Jetson para aplicaciones de mayor rendimiento.
\end{itemize}
...
En primer lugar, a Arturo Hernandez Rico, mi jefe de proyectos en GTD Defense \& Security, por la idea de llevar a término este aparato y una serie de alternativas ofrecidas.

Agradecer también a mi tutor Dr. Gabriel Guerrero Contreras por todos los buenos consejos, recomendaciones, paciencia, persistencia y disponibilidad facilitadas durante este ejercicio. Sus mensajes periodicos han sido determinantes para animarme a continuar desarrollando.

A continuación agradezco al Dr. Andrés Yañez Escolano de la Escuela Superior de Ingeniería del Campus de Puerto Real, el prestamo de una placa Raspberry Pi 3 Model B junto con una tarjeta de memoria durante la crisis de los semiconductores acaecida en los últimos meses, la cual ha convertido en una auténtica odisea y a precios desorbitados conseguir una pieza de hardware como la mencionada anteriormente para poder comenzar con el proyecto.

Al Dr. Alberto Paramio Leiva, Dra. Maria Ángeles Moreno Frías y al Prof. Juan Antonio García Órdoñez por realizarme una revisión con una opinión sincera del proyecto una vez terminado el documento y la implementación.

A mi amgio de la infancia, Adrián Crespo Delgado, por su ayuda en la construcción y diseño en 3D de una carcasa que pudiera albergar toda la circuitería del proyecto.

Aunque sin participación directa, me gustaría agradecer a mi familia y amigos por los ánimos proporcionados para acabar el proyecto. En especial, a mi prometida, Ana Pastor Muñoz, que ha aguantado sin poder disfrutar completamente sus vacaciones para que pueda sentarme a desarrollarlo y estudiar, por la paciencia que ha tenido conmigo, el esfuerzo realizado y todo el apoyo que me ha brindado, gracias de corazón.
...
\begin{enumerate}
    ...
    Este comando construye una imagen basada en Ubuntu 24.04 ARM64 que incluye:
    \begin{itemize}
...
\begin{enumerate}
    \item Instalar el compilador cruzado GCC 13 en el host:
    \begin{verbatim}
    sudo apt install gcc-13-aarch64-linux-gnu \
      g++-13-aarch64-linux-gnu
    \end{verbatim}
    
    \item Compilar el proyecto en modo stubs (para testing en Docker):
    \begin{verbatim}
    cd LatencyTester/Scripts/Linux
    ./build-arm64.sh
    \end{verbatim}
    
    El script detecta automáticamente el qmake de Qt ARM64 (\texttt{/opt/Qt/6.11.1/arm64/bin/qmake}), configura los compiladores cruzados GCC 13 y compila con \texttt{USE\_STUBS=1}. El binario resultante (\texttt{build\_arm64/LatencyTester}) es un ejecutable ELF ARM64 que utiliza stubs de hardware, apto para validación funcional sin dispositivo físico.
\end{enumerate}
...
\textbf{Nota}: La limitación de qmake bajo emulación QEMU que motiva esta arquitectura de compilación se describe en detalle en la sección de Implementación.
...
\texttt{tests} & Ejecuta los tests unitarios en modo offscreen dentro del contenedor. \\ \hline
\texttt{valgrind} & Ejecuta Valgrind con \texttt{--leak-check=full} sobre el binario en modo offscreen. \\ \hline
\texttt{shell} & Abre una sesión Bash interactiva dentro del contenedor. \\ \hline
...
El binario de producción se genera mediante compilación cruzada desde el host Linux, utilizando los drivers reales de hardware (pigpio para GPIO y libgpiod v2 para la detección de flancos del ADC). A diferencia del modo de pruebas (stubs), este binario interactúa directamente con el hardware de la Raspberry Pi.

\subsection{Prerequisitos (una sola vez)}

\begin{enumerate}
    \item \textbf{Compilador cruzado GCC 13} (necesario para soporte completo de C++20):
    \begin{verbatim}
    sudo apt install gcc-13-aarch64-linux-gnu \
      g++-13-aarch64-linux-gnu lld
    \end{verbatim}
    
    \item \textbf{Herramientas de compilación de librerías}:
    \begin{verbatim}
    sudo apt install autoconf automake libtool \
      autoconf-archive pkg-config
    \end{verbatim}
    
    \item \textbf{Imagen Docker ARM64 construida} (proporciona el sysroot):
    \begin{verbatim}
    cd LatencyTester/Scripts/Linux
    ./docker-rpi.sh build
    \end{verbatim}
    
    \item \textbf{Sysroot extraído del contenedor}:
    \begin{verbatim}
    ./extract-sysroot.sh
    \end{verbatim}
    El script extrae las librerías y cabeceras ARM64 a \texttt{\textasciitilde/ARM64-sysroot/}, crea los enlaces simbólicos necesarios para el linker (incluyendo \texttt{ld-linux-aarch64.so.1}) y descarga la cabecera \texttt{pigpio.h} desde GitHub.
    
    \item \textbf{Qt 6.11.1 compilado para ARM64} (proceso de unas 2 horas):
    \begin{verbatim}
    ./build-qt6-arm64.sh
    \end{verbatim}
    Instala Qt ARM64 en \texttt{/opt/Qt/6.11.1/arm64/}.
    
    \item \textbf{libgpiod v2.2 con bindings C++} (no disponible como paquete en Ubuntu 24.04):
    \begin{verbatim}
    ./build-libgpiod-arm64.sh
    \end{verbatim}
    Compila libgpiod v2.2 desde fuente con \texttt{--enable-bindings-cxx} e instala \texttt{libgpiodcxx.so} y las cabeceras v2 (\texttt{gpiod.hpp}, \texttt{gpiodcxx/*.hpp}) en el sysroot.
    
    \item \textbf{pigpio} (librería GPIO específica de Raspberry Pi, no disponible en repos):
    \begin{verbatim}
    ./build-pigpio-arm64.sh
    \end{verbatim}
    Compila pigpio desde fuente e instala \texttt{libpigpio.so} en el sysroot.
\end{enumerate}

\subsection{Generación del binario de producción}

Una vez completados los prerequisitos, la compilación del binario de producción se reduce a un solo comando:
...
Este comando:
\begin{itemize}
    \item Utiliza el qmake de Qt ARM64 (\texttt{/opt/Qt/6.11.1/arm64/bin/qmake}).
    \item Invoca GCC 13 como compilador cruzado (\texttt{aarch64-linux-gnu-g++-13}).
    \item Compila con \texttt{USE\_STUBS=0}, activando la macro \texttt{RASPBERRY\_PI} y enlazando contra los drivers reales.
    \item Genera el binario en \texttt{build\_arm64/LatencyTester}.
\end{itemize}

El binario resultante enlaza dinámicamente contra:
\begin{itemize}
    \item \texttt{libpigpio.so.1} — control GPIO (LED estimulador).
    \item \texttt{libgpiod.so} + \texttt{libgpiodcxx.so} — detección de flancos por interrupción (señal ALERT/RDY del ADS1115).
    \item Librerías Qt 6.11.1 ARM64 (Widgets, Concurrent, PrintSupport, VirtualKeyboard).
\end{itemize}

\subsection{Transferencia a la Raspberry Pi}

\begin{verbatim}
scp build_arm64/LatencyTester pi@<ip-rpi>:/home/pi/LatencyTester/bin/
\end{verbatim}

\subsection{Dependencias en la Raspberry Pi}

En la Raspberry Pi de destino deben estar instaladas las librerías de runtime:
\begin{verbatim}
sudo apt install libqt6widgets6 libqt6concurrent6 \
  libqt6printsupport6 qt6-qpa-plugins libgpiod2
\end{verbatim}

Adicionalmente, \texttt{pigpio} debe instalarse en la Raspberry Pi:
\begin{verbatim}
sudo apt install pigpio     # si disponible en Raspi OS
# O compilar desde fuente:
wget https://github.com/joan2937/pigpio/archive/master.tar.gz
tar xzf master.tar.gz && cd pigpio-master
...
\texttt{saveMeasureToDisk(\-measures)} & Serializa una estructura \texttt{Measures} a JSON y la escribe en disco. El nombre del fichero se genera automáticamente a partir de la fecha ISO (con dos puntos reemplazados por guiones para compatibilidad Windows). \\ \hline
...
\texttt{HelpScreen} & Botón «Manual de usuario» se habilita o deshabilita según exista el recurso HTML embebido para el idioma activo. \\ \hline
...
\texttt{RegistryDisplayScreen} & Muestra datos de una medición guardada y gráfica con la misma lógica de \texttt{plotMeasure}. \\ \hline
\end{tabular}
\end{table}
...
\label{tab:dev_tests}
\begin{tabular}{|p{4.5cm}|p{1cm}|p{6cm}|}
\hline
\textbf{Suite} & \textbf{Tests} & \textbf{Ámbito} \\ \hline
\texttt{tst\_datamodel} & 17 & Valores por defecto, casos extremos de structs. \\ \hline
...
\texttt{tst\_registrydisplay\-screen} & 14 & displayMeasure, ploteo, casos extremos. \\ \hline
\hline
